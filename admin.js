...
async function saveData(key, val) {
  binData[key] = val;
  binData._lastUpdated = Date.now(); // stamp every save so visitors know cache is stale
...
async function fetchAndInitBin() {
  ...
  } catch (e) { /* network error — keep binData as {} */ }

  // ONLY initialize with defaults if fetch actually succeeded
  // (prevents overwriting real data when network fails)
  if (!fetchOk) {
    showToast('Could not reach server — retrying…', true);
    // Retry once after 2s
    await new Promise(r => setTimeout(r, 2000));
    try {
      const res2 = await fetch(BIN_URL, { headers: { 'X-Master-Key': WRITE_KEY } });
      if (res2.ok) { binData = (await res2.json()).record || {}; fetchOk = true; }
    } catch (e) {}
    if (!fetchOk) { showToast('Server unreachable — working offline.', true); return; }
  }

  let needsSave = false;

  // First-time setup: only if bin has never been initialized
  if (!binData._initialized) {
    if (!binData.news     || !binData.news.length)     { binData.news     = DEFAULT_NEWS;     needsSave = true; }
    if (!binData.fixtures || !binData.fixtures.length) { binData.fixtures = DEFAULT_FIXTURES; needsSave = true; }
    if (!binData.squad    || !binData.squad.length)    { binData.squad    = DEFAULT_SQUAD;    needsSave = true; }
    if (!binData.sponsors || !binData.sponsors.length) { binData.sponsors = DEFAULT_SPONSORS; needsSave = true; }
    if (!binData.gallery  || !binData.gallery.length)  { binData.gallery  = DEFAULT_GALLERY;  needsSave = true; }
    if (!binData.settings || !Object.keys(binData.settings).length) { binData.settings = DEFAULT_SETTINGS; needsSave = true; }
    binData._initialized = true;
    needsSave = true;
  }

  // Strip any leftover base64 from squad/gallery (use ImgBB URLs instead)
  if (binData.squad) {
    binData.squad = binData.squad.map(p => {
      if (p.photo && p.photo.startsWith('data:')) { needsSave = true; return { ...p, photo: '' }; }
      return p;
    });
  }
  if (binData.gallery) {
    const before = binData.gallery.length;
    binData.gallery = binData.gallery.filter(item => !(item.src && item.src.startsWith('data:')));
    if (binData.gallery.length !== before) needsSave = true;
  }
...
function renderFixtures() {
  ...
  const results  = fixtures.filter(f => f.status !== 'upcoming');

  const upcomingTbody = document.getElementById('upcomingTable');
  const resultsTbody  = document.getElementById('resultsTable');
  const upcomingEmpty = document.getElementById('upcomingEmpty');
  const resultsEmpty  = document.getElementById('resultsEmpty');

  if (upcoming.length === 0) {
    upcomingTbody.innerHTML = ''; upcomingEmpty.style.display = 'block';
  } else {
    upcomingEmpty.style.display = 'none';
    upcomingTbody.innerHTML = upcoming.map(f => `
      <tr>
        <td>${f.day} ${f.month}</td>
        ...
        <td><div class="tbl-actions">
          <button class="tbl-btn edit" onclick="editFixture(${f.id})"><i class="fas fa-edit"></i></button>
          <button class="tbl-btn delete" onclick="deleteFixture(${f.id})"><i class="fas fa-trash"></i></button>
        </div></td>
      </tr>`).join('');
  }

  if (results.length === 0) {
    resultsTbody.innerHTML = ''; resultsEmpty.style.display = 'block';
  } else {
    resultsEmpty.style.display = 'none';
    resultsTbody.innerHTML = results.map(f => `
      <tr>
        <td>${f.day} ${f.month}${f.year ? ' ' + f.year : ''}</td>
        <td><strong>${escHtml(f.homeTeam)}</strong></td>
        <td><strong>${escHtml(f.awayTeam)}</strong></td>
        <td><strong>${escHtml(f.score)}</strong></td>
...
function openAddFixture(mode = 'upcoming') {
  const title = mode === 'result' ? 'Add Result' : 'Add Upcoming Fixture';
  openModal(title, fixtureForm(null, mode));
...
function editFixture(id) {
  ...
  if (!item) return;
  const mode = item.status === 'upcoming' ? 'upcoming' : 'result';
  openModal('Edit ' + (mode === 'result' ? 'Result' : 'Fixture'), fixtureForm(item, mode));
...
function fixtureForm(f, mode = 'upcoming') {
  const monthOpts = MONTHS.map((m, i) =>
    `<option value="${m}" ${f && f.month === m ? 'selected' : ''}>${m}</option>`
  ).join('');
  const defaultStatus = f ? f.status : (mode === 'result' ? 'win' : 'upcoming');
  const statusOpts = ['upcoming','win','loss','draw'].map(s =>
    `<option value="${s}" ${defaultStatus === s ? 'selected' : ''}>${s.charAt(0).toUpperCase()+s.slice(1)}</option>`
  ).join('');
  return `
    <form id="fixtureFormEl">
      <div class="modal-body form-row" style="padding:0;margin-bottom:0;">
        <div class="form-group">
          <label>${mode === 'result' ? 'Match Day' : 'Day'}</label>
        ...
        <div class="form-group" id="yearGroup" style="display:${mode === 'result' ? 'block' : 'none'};">
          <label>Year</label>
          <input type="number" id="fYear" placeholder="e.g. 2025" min="2020" max="2035" value="${f ? (f.year || new Date().getFullYear()) : new Date().getFullYear()}" />
        </div>
      </div>
      <div class="form-group">
...
function toggleScoreField() {
  ...
  if (!statusEl) return;
  const isResult = statusEl.value !== 'upcoming';
  if (scoreGroup) scoreGroup.style.display = isResult ? 'block' : 'none';
  if (yearGroup) yearGroup.style.display = isResult ? 'block' : 'none';
}
...
async function submitFixture(e, editId = null) {
  ...
  const item = {
    ...
    year: document.getElementById('fYear') ? document.getElementById('fYear').value.trim() : '',
  };
  if (editId) {
...
function renderSquad() {
  const squad = [...getData('squad', [])].sort((a, b) => parseInt(a.number || 0) - parseInt(b.number || 0));
...
async function readPhotoFile(inputId) {
  ...
  const json = await res.json();
  if (!json.success || !json.data) throw new Error('ImgBB error: ' + JSON.stringify(json));
  // Use the most reliable direct-image URL field
...
function openAddPlayer() {
  ...
  document.getElementById('playerFormEl').addEventListener('submit', async (e) => {
    ...
    btn.disabled = true;
    let photoData = null;
    const hasFile = document.getElementById('pPhotoFile')?.files?.length > 0;
    if (hasFile) {
      btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Uploading photo...';
      try {
        photoData = await readPhotoFile('pPhotoFile');
      } catch (err) {
        btn.disabled = false; btn.innerHTML = '<i class="fas fa-save"></i> Add Player';
        showToast('Photo upload failed — check internet connection and try again.', true);
    ...
    try {
      ...
      showToast(photoData ? 'Player added! Photo is now live for all visitors.' : 'Player added!');
    } catch {
      btn.disabled = false; btn.innerHTML = '<i class="fas fa-save"></i> Add Player';
      showToast('Failed to save — check connection.', true);
    }
...
function editPlayer(id) {
  ...
  document.getElementById('playerFormEl').addEventListener('submit', async (e) => {
    ...
    if (hasFile) {
      btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Uploading photo...';
      try {
        newPhotoData = await readPhotoFile('pPhotoFile');
      } catch (err) {
    ...
    try {
      ...
      showToast(newPhotoData ? 'Player updated! Photo is now live for all visitors.' : 'Player updated!');
    } catch {
      btn.disabled = false; btn.innerHTML = '<i class="fas fa-save"></i> Save Changes';
