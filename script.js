...
const CACHE_KEY = 'ufc_bin_cache';
const CACHE_TS  = 'ufc_bin_ts';
...
function renderAll(d) {
  ...
  if (squadEl && squad.length > 0) {
    const sortedSquad = [...squad].sort((a, b) => parseInt(a.number || 0) - parseInt(b.number || 0));
    squadEl.innerHTML = sortedSquad.map((p) => {
  ...
  if (fixEl && fixtures.length > 0) {
    ...
    function fixtureCard(f) {
      ...
      return `<div class="fixture-item ${isPast ? 'past' : ''} reveal">
        <div class="fixture-date">
          ...
          ${isPast && f.year ? `<span class="fix-year">${f.year}</span>` : ''}
        </div>
    ...
    if (upcoming.length > 0) {
      html += `<div class="fixtures-group">
        <div class="fixture-group-label upcoming"><i class="fas fa-calendar-alt"></i> Upcoming Fixtures</div>
        ${upcoming.map(fixtureCard).join('')}
      </div>`;
    }
    if (results.length > 0) {
      const shown  = results.slice(0, RESULTS_LIMIT);
      const hidden = results.slice(RESULTS_LIMIT);
      html += `<div class="fixtures-group">
        <div class="fixture-group-label results"><i class="fas fa-flag-checkered"></i> Results</div>
        ${shown.map(fixtureCard).join('')}
        ${hidden.length > 0 ? `
          <div id="extra-results" style="display:none;">${hidden.map(fixtureCard).join('')}</div>
          <button class="show-more-results" onclick="
            var el=document.getElementById('extra-results');
            var shown=el.style.display!=='none';
            el.style.display=shown?'none':'block';
            this.innerHTML=shown
              ?'<i class=\\'fas fa-chevron-down\\'></i> Show ${hidden.length} more results'
              :'<i class=\\'fas fa-chevron-up\\'></i> Show less';
          "><i class="fas fa-chevron-down"></i> Show ${hidden.length} more result${hidden.length > 1 ? 's' : ''}</button>
        ` : ''}
      </div>`;
    }
...
async function loadDynamicContent() {
  // 1. Show cache immediately if it's fresh (under 3 min old)
  const cached   = localStorage.getItem(CACHE_KEY);
  const cachedTs = parseInt(localStorage.getItem(CACHE_TS) || '0');
  const cacheAge = Date.now() - cachedTs;

  if (cached && cacheAge < CACHE_TTL) {
    try { renderAll(JSON.parse(cached)); } catch (e) {}
  } else {
    // Cache is stale or missing — show defaults while we fetch
    renderAll({});
  }
